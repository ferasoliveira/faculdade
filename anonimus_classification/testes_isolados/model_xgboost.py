...
# --- Adicione esta linha para ajustar os rótulos da classe para serem 0-indexados ---
# Se suas classes são 1, 2, 3, 4, elas se tornarão 0, 1, 2, 3
y = y - 1 
# ----------------------------------------------------------------------------------
...
melhor_resultado_xgb = {'accuracy': 0}

for n_estimators in n_estimators_list:
    for learning_rate in learning_rate_list:
        for max_depth in max_depth_list:
            print(f"\n>> Treinando XGBoost: n_estimators={n_estimators}, learning_rate={learning_rate}, max_depth={max_depth}")
            t0 = time.time()
            ...
            clf = XGBClassifier(
                ...
                eval_metric='mlogloss' if y.nunique() > 2 else 'logloss', # Use mlogloss for multi-class
                random_state=42,
                n_jobs=-1
            )
            ...
            if acc > melhor_resultado_xgb['accuracy']:
...
all_results['XGBoostClassifier'] = melhor_resultado_xgb

with open(json_path, 'w') as f:
    json.dump(all_results, f, indent=4)

print(f"Melhor resultado para XGBoost salvo em: {json_path}")
